...
class CustomImageFolder(ImageFolder):
    """
    Custom ImageFolder dataset which returns 1 image, 1 target, and image index.
    """
    ...
    def __getitem__(self, index):
        ...
        # Return image, target, and index
        return original_img, [target, index]
...
def load_dataset(args):
    ...
    if name == 'CIFAR-100':
        ...
        if args.method == 'DDGSD':
            trainset = datasets.CIFAR10(root, train=True,  download=True, transform=TwoCropsTransform(transform_train))
        elif args.method == 'PSKD':
        ...
        if args.method == 'DDGSD':
            ...
            trainset = CustomImageFolder(root=train_val_dataset_dir, transform=transform_train)
        else:
            trainset = datasets.ImageFolder(root=train_val_dataset_dir, transform=transform_train)
